/*
 * ClusterHealthMonitor.actor.cpp
 *
...
ACTOR Future<LatestWorkerEvents> latestEventOnWorker(WorkerInterface worker, std::string eventName) {
	try {
		state EventLogRequest req =
		    eventName.empty() ? EventLogRequest() : EventLogRequest(Standalone<StringRef>(eventName));
		state ErrorOr<TraceEventFields> traceEvent =
		    wait(errorOr(timeoutError(worker.eventLogRequest.getReply(req), 2.0)));

		state WorkerEvents results;
		state std::set<std::string> failed;
...
ACTOR template <class Interface>
Future<LatestWorkerEvents> latestEventOnInterfaces(std::vector<Interface> interfaces,
	...
	try {
		...
		for (i = 0; i < interfaces.size(); ++i) {
			auto workerIt = addressWorkers.find(interfaces[i].address());
			if (workerIt == addressWorkers.end()) {
				failed.insert(interfaces[i].address().toString());
				results[interfaces[i].address()] = TraceEventFields();
				continue;
			}

			addresses.push_back(interfaces[i].address());
			eventTraces.push_back(
			    errorOr(timeoutError(workerIt->second.eventLogRequest.getReply(EventLogRequest(
			                             Standalone<StringRef>(interfaces[i].id().toString() + "/" + eventName))),
			                         2.0)));
		}

		wait(waitForAll(eventTraces));

		for (i = 0; i < eventTraces.size(); ++i) {
...
ACTOR Future<Void> monitorRun(Monitor* self) {
	if (!SERVER_KNOBS->CLUSTER_HEALTH_METRIC_ENABLE) {
		return Void();
	}

	state Future<Void> timer = Void();
	state std::vector<Future<Level>> levelFutures;
	state Optional<int> limitingIndex;
	state Level limitingLevel;
	state int i;
	loop {
		wait(timer);
		...
		levelFutures.clear();
		levelFutures.reserve(self->getFactors().size());
		for (i = 0; i < self->getFactors().size(); ++i) {
			levelFutures.push_back(
			    self->getFactors()[i]->fetchLevel(self->getWorkerEventProvider(), TrackCodeProbes::True));
		}
		wait(waitForAll(levelFutures));

		limitingIndex = Optional<int>();
		limitingLevel = Level::HEALTHY;
		TraceEvent traceEvent("ClusterHealthMetric");
		for (i = 0; i < self->getFactors().size(); ++i) {
			Level level = levelFutures[i].get();
			traceEvent.detail(fmt::format("Factor{}", self->getFactors()[i]->getName()), levelToStr(level));
		...
		if (limitingIndex.present() && limitingLevel != Level::HEALTHY) {
			traceEvent.detail("LimitingFactor", self->getFactors()[limitingIndex.get()]->getName());
		}
	}
}

Future<Void> Monitor::run() {
	return monitorRun(this);
}
