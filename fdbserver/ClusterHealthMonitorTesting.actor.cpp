/*
 * ClusterHealthMonitorTesting.actor.cpp
 *
...
ACTOR Future<Void> assertFactorLevel(IFactor* factor,
                                     Reference<IWorkerEventProvider const> provider,
                                     Level expectedLevel) {
	state Level level = wait(factor->fetchLevel(provider, TrackCodeProbes::False));
	ASSERT_EQ(level, expectedLevel);
	return Void();
}
...
TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/StorageSpaceFactor") {
	state StorageSpaceFactor factor(/*interventionThreshold=*/0.20, /*criticalInterventionThreshold=*/0.10);
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();
	...
	wait(assertFactorLevel(&factor, provider, Level::HEALTHY));

	provider->setLatestStorageServerEvents(
	    "StorageMetrics",
	    makeLatestWorkerEvents(makeSpaceMetrics("KvstoreBytesAvailable", "KvstoreBytesTotal", 15, 100)));
	wait(assertFactorLevel(&factor, provider, Level::INTERVENTION_REQUIRED));

	provider->setLatestStorageServerEvents(
	    "StorageMetrics",
	    makeLatestWorkerEvents(makeSpaceMetrics("KvstoreBytesAvailable", "KvstoreBytesTotal", 5, 100)));
	wait(assertFactorLevel(&factor, provider, Level::CRITICAL_INTERVENTION_REQUIRED));

	state WorkerEvents mixedRoleEvents;
	...
	wait(assertFactorLevel(&factor, provider, Level::METRICS_MISSING));
	return Void();
}

TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/TLogSpaceFactor") {
	state TLogSpaceFactor factor(/*interventionThreshold=*/0.20, /*criticalInterventionThreshold=*/0.10);
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();
...
TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/StorageReplicationFactor") {
	state StorageReplicationFactor factor;
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();
	...
	wait(assertFactorLevel(&factor, provider, Level::SELF_HEALING));
	...
	wait(assertFactorLevel(&factor, provider, Level::OUTAGE));

	state WorkerEvents staleWorkerEvents;
...
TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/RecoveryStateFactor") {
	state RecoveryStateFactor factor;
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();
	...
	state Reference<FakeWorkerEventProvider> missingProvider = makeReference<FakeWorkerEventProvider>();
	wait(assertFactorLevel(&factor, missingProvider, Level::METRICS_MISSING));
	return Void();
}

TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/ProcessErrorsFactor") {
	state ProcessErrorsFactor factor;
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();

	provider->setLatestEvents("", makeLatestWorkerEvents(makeProcessErrorMetrics("OpenClusterIdError", "io_error")));
	wait(assertFactorLevel(&factor, provider, Level::CRITICAL_INTERVENTION_REQUIRED));

	state WorkerEvents emptyLatestErrors;
	...
	state WorkerEvents partiallyFailedLatestErrors;
...
TEST_CASE("/fdbserver/clustercontroller/ClusterHealthMonitor/RkThrottlingFactor") {
	state RkThrottlingFactor factor(/*criticalTpsLimitToReleasedTpsRatioThreshold=*/1.20);
	state Reference<FakeWorkerEventProvider> provider = makeReference<FakeWorkerEventProvider>();
