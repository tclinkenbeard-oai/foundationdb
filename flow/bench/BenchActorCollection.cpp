...
Future<Void> benchActorCollectionConstructCancel(benchmark::State* state) {
	for (auto _ : *state) {
		...
		Future<Void> collection = makeActorCollection(addActor);
...
Future<Void> benchActorCollectionReadyChild(benchmark::State* state) {
	for (auto _ : *state) {
		benchmark::DoNotOptimize(_);
		PromiseStream<Future<Void>> addActor;
		Future<Void> collection = makeActorCollection(addActor, nullptr, true);
...
Future<Void> benchActorCollectionBatchComplete(benchmark::State* state) {
	...
	for (auto _ : *state) {
		...
		PromiseStream<Future<Void>> addActor;
		int count = 0;
		Future<Void> collection = makeActorCollection(addActor, &count, true);
...
Future<Void> benchActorCollectionBatchCancel(benchmark::State* state) {
	...
	for (auto _ : *state) {
		...
		PromiseStream<Future<Void>> addActor;
		int count = 0;
		Future<Void> collection = makeActorCollection(addActor, &count);
		std::vector<Promise<Void>> signals(batchSize);
		for (auto& signal : signals) {
...
void bench_actorCollection_error_child(benchmark::State& state) {
	onMainThread([&state] { return benchActorCollectionErrorChild(&state); }).blockUntilReady();
}
...
BENCHMARK(bench_actorCollection_error_child)->Name("ActorCollection/error_child")->ReportAggregatesOnly(true);
