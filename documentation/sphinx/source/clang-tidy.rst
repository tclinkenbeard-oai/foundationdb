...
FoundationDB enables 25 checks configured in the ``.clang-tidy`` file at the repository root. The
intent is to enable more as we go forward. Here are some example rules:

* **11 Bugprone rules** -- catch potential runtime errors (e.g., ``bugprone-use-after-move``, ``bugprone-switch-missing-default-case``)
* **2 Miscellaneous rules** -- catch redundant expressions and unsafe RAII objects across coroutine suspension points
