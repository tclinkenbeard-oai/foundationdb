...
Future<Version> waitForCommittedVersion(Database cx, Version version, SpanContext spanContext) {
	...
	while (true) {
		Error err;
		try {
			auto res = co_await race(cx->onProxiesChanged(),
		...
		if (err.code() == error_code_batch_transaction_throttled ||
		    err.code() == error_code_grv_proxy_memory_limit_exceeded) {
			// GRV Proxy returns an error
			co_await delayJittered(CLIENT_KNOBS->GRV_ERROR_RETRY_DELAY);
		} else {
			TraceEvent(SevError, "WaitForCommittedVersionError").error(err);
			throw err;
		}
	}
}
